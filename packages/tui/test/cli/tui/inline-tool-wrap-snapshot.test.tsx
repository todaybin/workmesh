...
describe("TUI inline tool wrapping", () => {
  ...
  test("formats fixed subagent labels in Chinese", () => {
    expect(formatSubagentToolcalls(2, "zh-CN")).toBe("2 次工具调用")
    expect(formatSubagentTitle("Explore", "检查渲染器", true, "zh-CN")).toBe("Explore 任务（后台） - 检查渲染器")
    expect(formatSubagentRetry(2, "请求受限", "zh-CN")).toBe("正在重试（第 2 次） · 请求受限")
  })
