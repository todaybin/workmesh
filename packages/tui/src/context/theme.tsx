...
const [store, setStore] = createStore<State>({
  ...
  mode: "dark",
  lock: undefined,
  active: TuiProduct.defaultTheme,
  ready: false,
})
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", TuiProduct.defaultTheme)
        draft.active = typeof active === "string" ? active : TuiProduct.defaultTheme
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", TuiProduct.defaultTheme))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", TuiProduct.defaultTheme)
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes[TuiProduct.defaultTheme], store.mode)
    })
