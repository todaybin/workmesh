...
const bold = "\x1b[1m"
const dim = "\x1b[90m"
const workmeshPrimary = "\x1b[38;2;216;135;58m"

function wordmark(pad = "") {
  if (TuiProduct.enabled) return workmesh.lines.map((line) => `${pad}${workmeshPrimary}${line.trimEnd()}${reset}`)
...
export function sessionEpilogue(input: { title: string; sessionID?: string }) {
  ...
  return [
    ...
    `  ${weak("Continue")}${bold}${TuiProduct.cliName} -s ${input.sessionID}${reset}`,
    "",
  ].join("\n")
}
