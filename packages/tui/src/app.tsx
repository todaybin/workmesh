...
import { WorkMeshLocaleProvider, useWorkMeshLocale } from "./workmesh/locale"
import { composeCommandArguments, formatComposeDialogKey, ComposeRunsProvider, useComposeRuns } from "./workmesh/compose"
import {
  DialogComposeApproval,
  DialogComposeFinish,
  type ComposeApprovalAction,
  type ComposeFinishAction,
} from "./component/dialog-compose"
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const [composeDialogKey, setComposeDialogKey] = createSignal<string>()

  async function runComposeCommand(
    runID: string,
    action: ComposeApprovalAction | ComposeFinishAction,
    detail?: string,
  ) {
    ...
    const argumentsValue = composeCommandArguments(runID, action, detail)
    ...
    if ("error" in result && result.error) {
      toast.show({ variant: "error", title: locale.t("composeCommandFailed"), message: errorMessage(result.error) })
    }
  }

  function handleComposeApproval(runID: string, action: ComposeApprovalAction) {
    if (action !== "revise") {
      dialog.clear()
      void runComposeCommand(runID, action)
      return
    }
    dialog.replace(() => (
      <DialogPrompt
        title={locale.t("composeRevisionTitle")}
        placeholder={locale.t("composeRevisionPlaceholder")}
        onConfirm={(value) => {
          const detail = value.trim()
          if (!detail) return
          dialog.clear()
          void runComposeCommand(runID, action, detail)
        }}
      />
    ))
  }

  async function handleComposeFinish(runID: string, action: ComposeFinishAction) {
    if (action === "discard") {
      const confirmed = await DialogConfirm.show(
        dialog,
        locale.t("composeDiscardTitle"),
        locale.t("composeDiscardMessage"),
      )
  ...
  createEffect(() => {
    ...
    const run = Object.values(compose.runs())
      .filter((item) => item.sessionID === sessionID)
      .toSorted((a, b) => (b.updatedAt ?? 0) - (a.updatedAt ?? 0))[0]
    if (!run || (run.status !== "awaiting_approval" && run.status !== "awaiting_finish")) return
    const key = formatComposeDialogKey(run, compose.dialogRevision(run.id))
    if (composeDialogKey() === key) return
    setComposeDialogKey(key)
    if (run.status === "awaiting_approval") {
      dialog.replace(() => (
        <DialogComposeApproval run={run} onSelect={(action) => handleComposeApproval(run.id, action)} />
      ))
      return
    }
    dialog.replace(() => (
      <DialogComposeFinish run={run} onSelect={(action) => void handleComposeFinish(run.id, action)} />
    ))
  })
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle(TuiProduct.displayName)
      return
    }
