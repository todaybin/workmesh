...
import type { WorkMeshLocale } from "../../workmesh/command-locale"
    ...
    <>
      ...
      <Show when={props.parts.some((x) => x.type === "tool" && x.tool === "task")}>
        <box paddingTop={1} paddingLeft={3}>
          <text fg={theme.text}>
            {childShortcut()}
            <span style={{ fg: theme.textMuted }}> {locale.t("viewSubagents")}</span>
            <Show
              when={
                sync.data.capabilities.experimentalBackgroundSubagents &&
                props.parts.some(
            ...
            >
              ...
              {backgroundShortcut()}
              <span style={{ fg: theme.textMuted }}> {locale.t("background")}</span>
            </Show>
          </text>
        </box>
      </Show>
...
function ReasoningHeader(props: {
  ...
  const fg = () =>
    props.open
      ...
      : theme.warning
  const completed = () => {
    const thought = locale.t("thought")
    ...
    <Switch>
      <Match when={!props.done}>
        <box flexDirection="row">
          <Spinner color={fg()}>{props.title ? `${locale.t("thinking")}: ${props.title}` : locale.t("thinking")}</Spinner>
        </box>
      </Match>
      <Match when={true}>
...
function Task(props: ToolProps) {
  ...
  const content = createMemo(() => {
    ...
    if (isRunning() && retrying) {
      content.push(`↳ ${formatSubagentRetry(retrying.attempt, Locale.truncate(retrying.message, 80), locale.locale())}`)
    } else if (isRunning() && tools().length > 0) {
      if (current()) {
        const state = current()!.state
        const title = state.status === "running" || state.status === "completed" ? state.title : undefined
        content.push(`↳ ${Locale.titlecase(current()!.tool)} ${title}`)
      } else content.push(`↳ ${formatSubagentToolcalls(tools().length, locale.locale())}`)
    }

    if (!isRunning() && props.part.state.status === "completed") {
      content.push(`↳ ${formatCompletedSubagentDetail(tools().length, Locale.duration(duration()), locale.locale())}`)
    }
  ...
  return (
    <InlineTool
      ...
      onClick={() => {
        ...
        const status = retry()
        if (status) void DialogAlert.show(dialog, locale.t("retryError"), status.message)
      }}
    >
      {content()}
    </InlineTool>
...
export function formatSubagentToolcalls(count: number, locale: WorkMeshLocale = "en") {
  if (locale === "zh-CN") return `${count} 次工具调用`
...
export function formatSubagentTitle(
  ...
  description: string,
  background: boolean,
  locale: WorkMeshLocale = "en",
) {
  if (locale === "zh-CN") return `${agent} 任务${background ? "（后台）" : ""} - ${description}`
  return `${agent} Task${background ? " (background)" : ""} — ${description}`
}

export function formatSubagentRetry(attempt: number, message: string, locale: WorkMeshLocale = "en") {
  if (locale === "zh-CN") return `正在重试（第 ${attempt} 次） · ${message}`
  return `Retrying (attempt ${attempt}) · ${message}`
}

export function formatCompletedSubagentDetail(toolcalls: number, duration: string, locale: WorkMeshLocale = "en") {
