...
import { localizeSlashDescription } from "../../workmesh/command-locale"
...
export function Autocomplete(props: {
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    const localSlashes = slashes()
    const localNames = new Set(
      localSlashes.flatMap((command) => [command.display, ...(command.aliases ?? [])].map((item) => item.trim())),
    )
    const results: AutocompleteOption[] = localSlashes.map((command) => ({
      ...command,
      description: localizeSlashDescription(command.display.trim().slice(1), command.description, locale.locale()),
    }))

    for (const serverCommand of sync.data.command) {
      // TUI-native commands open dialogs immediately and must win over same-name server commands.
      if (localNames.has(`/${serverCommand.name}`)) continue
      const label = serverCommand.source === "mcp" ? ":mcp" : ""
      results.push({
        display: "/" + serverCommand.name + label,
        description: serverCommand.description,
        onSelect:
          serverCommand.name === "language" || serverCommand.name === "lang"
