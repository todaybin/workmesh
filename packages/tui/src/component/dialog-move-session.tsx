...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    const main = projectContext.data.project.mainDir
    if (directories.loading && !data && !current) return [{ title: locale.t("worktreeLoading"), value: undefined }]
    const createOption = {
      title: locale.t("worktreeCreate"),
      description: locale.t("worktreeCreateDescription"),
      value: { type: "new" } as const,
      category: locale.t("worktreeCategoryCreate"),
    }
    const roots = [...(data ?? [])]
    ...
    if (roots.length === 0) return [createOption]
      ...
      ...list.map((item) => {
        ...
        return {
          ...
          titleView: isRemoving ? (
            <span style={{ fg: theme.error }}>{locale.t("worktreeDeleting", { directory: item.location })}</span>
          ) : deleting ? (
            <span style={{ fg: theme.text }}>
              {locale.t("worktreeConfirmDelete", { shortcut: deleteHint() })}
            </span>
          ) : suffix ? (
            <>
          ...
          category:
            item.root.directory === current
              ? locale.t("worktreeCategoryCurrent")
              : item.root.directory === main
                ? locale.t("worktreeCategoryMain")
                : locale.t("worktreeCategoryOther"),
          titleWidth,
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, status?.data ?? [], {
          title: locale.t("worktreeDeleteTitle"),
          message: locale.t("worktreeDeleteMessage"),
        ...
        if (forced.error) {
          toast.show({
            variant: "error",
            title: locale.t("worktreeDeleteFailed"),
  ...
  return (
    <box minHeight={showError() ? 5 : fullHeight()}>
      <DialogSelect
        title={props.title ?? locale.t("moveSessionTitle")}
        titleView={
          <box flexDirection="row" gap={1}>
            <text fg={theme.text} attributes={TextAttributes.BOLD}>
              {props.title ?? locale.t("moveSessionTitle")}
            </text>
            ...
            : [
                {
                  ...
                  title: locale.t("worktreeActionNew"),
                  onTrigger: () => props.onSelect({ type: "new" }),
                },
                {
                  command: "dialog.move_session.delete",
                  title: locale.t("worktreeActionDelete"),
                  disabled: (option) => {
                ...
                },
                {
                  ...
                  title: locale.t("worktreeActionRefresh"),
