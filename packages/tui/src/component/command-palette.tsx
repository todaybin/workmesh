...
export function CommandPaletteDialog() {
  ...
  const options = createMemo(() =>
    entries().map((entry) => ({
      title: localizeSlashDescription(
        entry.command.name.split(".").at(-1) ?? entry.command.name,
        typeof entry.command.title === "string" ? entry.command.title : entry.command.name,
        locale.locale(),
      ) ?? entry.command.name,
      description: localizeSlashDescription(
        entry.command.name.split(".").at(-1) ?? entry.command.name,
  ...
  return <DialogSelect ref={(value) => (ref = value)} title={locale.t("commands")} options={list()} />
}
