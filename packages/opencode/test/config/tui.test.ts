...
it.instance("WorkMesh ignores and does not migrate explicit .opencode config", () =>
  withCleanState(
    withEnv(
      "WORKMESH_BUILD",
      "1",
      Effect.gen(function* () {
        ...
        const legacyConfig = path.join(legacyDirectory, "legacy-opencode.json")
        const legacyTuiConfig = path.join(legacyDirectory, "tui-override.json")
        yield* fs.makeDirectory(legacyDirectory, { recursive: true })
        yield* fs.writeJson(legacyConfig, { model: "legacy/model", theme: "legacy-migrated-theme" })
        yield* fs.writeJson(legacyTuiConfig, { theme: "legacy-explicit-theme" })
        ...
        yield* withEnv(
          ...
          withEnv(
            ...
            Effect.gen(function* () {
              const config = yield* getTuiConfig(test.directory)
              expect(config.theme).not.toBe("legacy-explicit-theme")
              expect(JSON.parse(yield* fs.readFileString(legacyConfig)).theme).toBe("legacy-migrated-theme")
              expect(yield* fs.existsSafe(path.join(legacyDirectory, "tui.json"))).toBe(false)
            }),
          ),
        )
      }),
