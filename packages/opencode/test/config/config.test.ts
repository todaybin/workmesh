...
it.instance("WorkMesh loads branded configuration and ignores .opencode", () =>
  withProcessEnv(
    "WORKMESH_BUILD",
    "1",
    Effect.gen(function* () {
      const test = yield* TestInstance
      const branded = path.join(test.directory, ".workmesh", "config")
      yield* FSUtil.use.writeWithDirs(
        path.join(branded, "agents", "brand.md"),
        `---
model: test/model
mode: subagent
---
WorkMesh agent`,
      ...
      yield* FSUtil.use.writeWithDirs(
        path.join(test.directory, ".opencode", "agents", "legacy.md"),
        `---
model: test/model
mode: subagent
---
Legacy agent`,
      ...
      const legacyDirectory = path.join(test.directory, ".opencode")
      const legacyConfig = path.join(legacyDirectory, "explicit.jsonc")
      yield* writeConfigEffect(legacyDirectory, schemaConfig({ model: "legacy-directory/model" }), "opencode.jsonc")
      yield* FSUtil.use.writeWithDirs(legacyConfig, JSON.stringify(schemaConfig({ model: "legacy-explicit/model" })))
      yield* writeConfigEffect(test.directory, schemaConfig({ model: "legacy/model" }))
      yield* writeConfigEffect(test.directory, schemaConfig({ model: "workmesh/model" }), "workmesh.jsonc")

      yield* withProcessEnvs(
        { OPENCODE_CONFIG: legacyConfig, OPENCODE_CONFIG_DIR: legacyDirectory },
        withGlobalConfigDir(
          branded,
          Effect.gen(function* () {
            const config = yield* Config.use.get()
            const directories = yield* Config.use.directories()
            expect(config.model).toBe("workmesh/model")
            expect(config.agent?.brand?.prompt).toBe("WorkMesh agent")
            expect(config.agent?.legacy).toBeUndefined()
            expect(directories).toEqual([branded])
          }),
        ),
      )
    }),
