...
function withWorkMeshBuild<A, E, R>(enabled: boolean, effect: Effect.Effect<A, E, R>) {
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.WORKMESH_BUILD
      process.env.WORKMESH_BUILD = enabled ? "1" : "0"
      return previous
    }),
    () => effect,
    (previous) =>
...
describe("session.compaction.process", () => {
  itCompaction.instance(
    "omits the WorkMesh language prompt in official mode",
    () => {
      const stub = llm()
      let system: string[] = []
      stub.push(
  ...
  itCompaction.instance(
    "instructs the compaction model to use Chinese in WorkMesh mode",
    () => {
      const stub = llm()
      let system: string[] = []
      stub.push(
        reply("English model output remains unchanged", (input) => {
          system = input.system
        }),
      )
      return withWorkMeshBuild(
        ...
        Effect.gen(function* () {
          ...
          expect(system[0]).toContain("当前语言为简体中文")
          const compacted = (yield* ssn.messages({ sessionID: session.id })).find(
            (item) => item.info.role === "assistant" && item.info.summary,
          )
          expect(
            compacted?.parts.some(
              (part) => part.type === "text" && part.text === "English model output remains unchanged",
            ),
          ).toBe(true)
        }),
      ).pipe(withCompaction({ llm: stub.llmLayer }))
