...
import { WorkMeshLoops } from "../../src/session/loops"
...
function withWorkMesh<A, E, R>(effect: Effect.Effect<A, E, R>) {
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.WORKMESH_BUILD
    ...
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.WORKMESH_BUILD
        else process.env.WORKMESH_BUILD = previous
      }),
  )
}
...
noLLMServer.instance(
  "official mode omits WorkMesh commands and local state",
  () =>
    Effect.gen(function* () {
      ...
      expect((yield* commands.list()).map((command) => command.name)).not.toContain(Command.Default.LOOP)
      expect((yield* commands.list()).map((command) => command.name)).not.toContain(Command.Default.LOOPS)
      expect((yield* commands.list()).map((command) => command.name)).not.toContain(Command.Default.LANGUAGE)
      expect((yield* commands.list()).map((command) => command.name)).not.toContain(Command.Default.LANG)
      expect((yield* commands.list()).map((command) => command.name)).not.toContain(Command.Default.TERMINALS)
      expect((yield* commands.list()).map((command) => command.name)).not.toContain(Command.Default.MESSAGE)
      expect((yield* commands.list()).map((command) => command.name)).not.toContain(Command.Default.MESSAGES)
      yield* prompt.init()
      expect(yield* fs.existsSafe(path.join(test.directory, ".workmesh", "state", "workmesh-loops.json"))).toBe(false)
      expect(yield* fs.existsSafe(path.join(test.directory, ".workmesh", "config", "language.json"))).toBe(false)
    }),
  30_000,
)

noLLMServer.instance(
  "WorkMesh terminal commands send and acknowledge messages",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
        const sender = yield* sessions.create({ title: "Terminal A" })
        const recipient = yield* sessions.create({ title: "Terminal B" })
        const recipientTerminalId = `session:${recipient.id}`

        yield* prompt.command({ sessionID: recipient.id, command: Command.Default.MESSAGES, arguments: "" })
        const terminals = yield* prompt.command({ sessionID: sender.id, command: Command.Default.TERMINALS, arguments: "" })
        expect(terminals.parts.some((part) => part.type === "text" && part.text.includes(recipientTerminalId))).toBe(
          true,
        )

        const sent = yield* prompt.command({
          sessionID: sender.id,
          command: Command.Default.MESSAGE,
          arguments: `${recipientTerminalId} 接口字段已经更新`,
        })
        expect(sent.parts.some((part) => part.type === "text" && part.text.includes("消息已发送"))).toBe(true)

        const received = yield* prompt.command({
          sessionID: recipient.id,
          command: Command.Default.MESSAGES,
          arguments: "",
        })
        expect(received.parts.some((part) => part.type === "text" && part.text.includes("接口字段已经更新"))).toBe(
          true,
        )

        const empty = yield* prompt.command({
          sessionID: recipient.id,
          command: Command.Default.MESSAGES,
          arguments: "",
        })
        expect(empty.parts.some((part) => part.type === "text" && part.text.includes("当前没有未读消息"))).toBe(
          true,
        )
      }),
  ),
...
noLLMServer.instance(
  "language commands localize feedback and support the lang alias",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
        ...
        const sessions = yield* Session.Service
        const session = yield* sessions.create({})
        expect((yield* commands.get(Command.Default.INIT))?.description).toBe("引导创建或更新项目 AGENTS.md 规则")

        const current = yield* prompt.command({
          sessionID: session.id,
          command: Command.Default.LANGUAGE,
          arguments: "",
        })
        const currentText = current.parts.find((part) => part.type === "text")?.text ?? ""
        expect(currentText).toContain("当前界面与回复语言：中文")
        expect(currentText).toContain("用法：/language 中文|英文|自动")

        const changed = yield* prompt.command({
          sessionID: session.id,
          command: Command.Default.LANG,
          arguments: "en",
        })
        expect(
          changed.parts.some(
            (part) => part.type === "text" && part.text.includes("Interface and response language set to English"),
          ),
        ).toBe(true)
        expect((yield* commands.get(Command.Default.INIT))?.description).toBe("Guided AGENTS.md project setup")
        ...
        expect(invalid.parts.some((part) => part.type === "text" && part.text.includes("Unsupported language"))).toBe(
          true,
        )
      }),
    ),
...
noLLMServer.instance(
  "WorkMesh command list exposes built-in skills with localized descriptions",
  () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = process.env.WORKMESH_BUILTIN_SKILLS_DIR
        process.env.WORKMESH_BUILTIN_SKILLS_DIR = path.resolve(
          process.cwd(),
          "../../../desktop/resources/workmesh/builtin/skills",
        )
        return previous
      }),
      () =>
        withWorkMesh(
          Effect.gen(function* () {
            ...
            yield* prompt.command({ sessionID: session.id, command: Command.Default.LANGUAGE, arguments: "en" })
            const list = yield* commands.list()
            expect(list.some((command) => command.name === "docx" && command.source === "skill")).toBe(true)
            expect(list.find((command) => command.name === "docx")?.description).toMatch(/Word documents|Word 文档/)
            expect(list.find((command) => command.name === Command.Default.INIT)?.description).toContain("AGENTS.md")
          }),
        ),
      (previous) =>
        Effect.sync(() => {
          if (previous === undefined) delete process.env.WORKMESH_BUILTIN_SKILLS_DIR
          else process.env.WORKMESH_BUILTIN_SKILLS_DIR = previous
        }),
    ),
  { git: true },
  30_000,
...
it.instance(
  "injects the selected WorkMesh reply language into model system messages",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const { llm } = yield* useServerConfig(providerCfg)
        ...
        const execute = yield* agents.get("compose-execute")
        const review = yield* agents.get("compose-review")
        expect(Permission.evaluate("edit", "*", compose.permission).action).toBe("deny")
        expect(Permission.evaluate("bash", "*", compose.permission).action).toBe("deny")
        expect(Permission.evaluate("edit", "*", execute.permission).action).toBe("allow")
        ...
        const session = yield* sessions.create({ title: "Language preference" })
        ...
        yield* prompt.prompt({
          sessionID: session.id,
          agent: "build",
          parts: [{ type: "text", text: "check language" }],
        })

        const chinese = JSON.stringify((yield* llm.inputs).at(-1)?.messages)
        ...
        yield* llm.text("English reply")
        yield* prompt.prompt({
          sessionID: session.id,
          agent: "build",
          parts: [{ type: "text", text: "check language again" }],
        })

        const english = JSON.stringify((yield* llm.inputs).at(-1)?.messages)
        expect(english).toContain("The current language is English")
        expect(english).not.toContain("当前语言为简体中文")
      }),
  ),
  { git: true },
  30_000,
...
noLLMServer.instance(
  "loops command lists jobs in Chinese and cancels by ID prefix",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
        ...
        const sessions = yield* Session.Service
        const session = yield* sessions.create({})
        const first = yield* loops.create({
          sessionID: session.id,
          prompt: "检查测试报告",
          intervalSeconds: 60,
          agent: "build",
        ...
        const listed = yield* prompt.command({
          sessionID: session.id,
          command: Command.Default.LOOPS,
          arguments: "",
        })
        const text = listed.parts.find((part) => part.type === "text")?.text ?? ""
        expect(text).toContain("当前会话共有 2 个循环任务")
        expect(text).toContain("检查测试报告")
        expect(text).toContain("每 120 秒")

        const ambiguous = yield* prompt
          .command({ sessionID: session.id, command: Command.Default.LOOPS, arguments: "loop-" })
          .pipe(Effect.exit)
        expect(Exit.isFailure(ambiguous)).toBe(true)

        const cancelled = yield* prompt.command({
          sessionID: session.id,
          command: Command.Default.LOOPS,
          arguments: first.id.slice(0, -1),
        })
        expect(
          cancelled.parts.some((part) => part.type === "text" && part.text.includes(`已取消循环任务 ${first.id}`)),
        ).toBe(true)
...
it.instance(
  "locks Compose mode after the first real user message",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const { llm } = yield* useServerConfig(providerCfg)
        ...
        const buildSession = yield* sessions.create({ title: "Build mode lock" })
        yield* llm.text("Build 完成")
        yield* prompt.prompt({
          sessionID: buildSession.id,
          agent: "build",
          parts: [{ type: "text", text: "先使用 Build" }],
        })
        const enteringCompose = yield* Effect.exit(
          prompt.prompt({
            sessionID: buildSession.id,
            agent: "compose",
            parts: [{ type: "text", text: "切换 Compose" }],
          }),
        )
        expect(Exit.isFailure(enteringCompose)).toBe(true)
        if (Exit.isFailure(enteringCompose)) {
          const error = Cause.squash(enteringCompose.cause)
          expect(NamedError.Unknown.isInstance(error)).toBe(true)
          if (NamedError.Unknown.isInstance(error)) expect(error.data.message).toContain("不能中途切换为 Compose")
        }

        const composeSession = yield* sessions.create({ title: "Compose mode lock" })
        yield* llm.text("Compose 规格")
        yield* prompt.prompt({
          sessionID: composeSession.id,
          agent: "compose",
          parts: [{ type: "text", text: "先使用 Compose" }],
        })
        const internalAgent = yield* Effect.exit(
          prompt.prompt({
            sessionID: composeSession.id,
            agent: "compose-execute",
            parts: [{ type: "text", text: "尝试绕过 Compose 运行时" }],
            noReply: true,
          }),
        )
        expect(Exit.isFailure(internalAgent)).toBe(true)
        if (Exit.isFailure(internalAgent)) {
          const error = Cause.squash(internalAgent.cause)
          expect(NamedError.Unknown.isInstance(error)).toBe(true)
          if (NamedError.Unknown.isInstance(error)) expect(error.data.message).toContain("仅供 Compose 运行时内部调用")
        }
        const leavingCompose = yield* Effect.exit(
          prompt.prompt({
            sessionID: composeSession.id,
            agent: "build",
            parts: [{ type: "text", text: "切换 Build" }],
          }),
        )
        expect(Exit.isFailure(leavingCompose)).toBe(true)
        if (Exit.isFailure(leavingCompose)) {
          const error = Cause.squash(leavingCompose.cause)
          expect(NamedError.Unknown.isInstance(error)).toBe(true)
          if (NamedError.Unknown.isInstance(error)) expect(error.data.message).toContain("锁定为 Compose 模式")
        }
      }),
    ),
  { git: true },
...
it.instance(
  "keeps reserved Compose security fields locked against user and session overrides",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const agents = yield* AgentSvc.Service
        ...
        expect(Permission.evaluate("bash", "*", compose.permission).action).toBe("deny")
        expect(execute.hidden).toBe(true)
        expect(Permission.evaluate("task", "general", execute.permission).action).toBe("deny")
        expect(Permission.evaluate("bash", "pnpm test", execute.permission).action).toBe("deny")
        expect(Permission.evaluate("external_directory", "*", execute.permission).action).toBe("deny")
        expect(Permission.evaluate("mcp_git_commit", "*", execute.permission).action).toBe("deny")
        expect(Permission.evaluate("custom_process_tool", "*", execute.permission).action).toBe("deny")
        expect(Permission.evaluate("bash", "git commit -am bypass", execute.permission).action).toBe("deny")
        const merged = ComposePermission.merge(compose, Permission.fromConfig({ edit: "allow", bash: "allow" }))
        expect(Permission.evaluate("edit", "*", merged).action).toBe("deny")
...
it.instance(
  "requires an explicit dialog confirmation before Compose finish",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        ...
        const sessions = yield* Session.Service
        const test = yield* TestInstance
        const session = yield* sessions.create({ title: "Compose finish confirmation" })
        const compose = yield* Effect.promise(() => createComposeService({ directory: test.directory }))
        let run = yield* Effect.promise(() => compose.start({ task: "Confirm finish", sessionID: session.id }))
        run = yield* Effect.promise(() => compose.transition({ id: run.id, phase: "design" }))
        run = yield* Effect.promise(() => compose.transition({ id: run.id, phase: "awaiting_approval" }))
        run = yield* Effect.promise(() => compose.approve(run.id))
        run = yield* Effect.promise(() => compose.transition({ id: run.id, phase: "implement" }))
        run = yield* Effect.promise(() => compose.transition({ id: run.id, phase: "verify" }))
        run = yield* Effect.promise(() => compose.transition({ id: run.id, phase: "review" }))
        run = yield* Effect.promise(() => compose.transition({ id: run.id, phase: "finalize" }))
        run = yield* Effect.promise(() => compose.awaitFinish(run.id))

        const direct = yield* Effect.exit(
          prompt.command({
            sessionID: session.id,
            command: Command.Default.COMPOSE,
            arguments: `keep ${run.id}`,
            agent: "compose",
          }),
        )
        expect(Exit.isFailure(direct)).toBe(true)
        if (Exit.isFailure(direct)) expect(Cause.pretty(direct.cause)).toContain("Finish 弹窗")
        expect((yield* Effect.promise(() => compose.get(run.id))).status).toBe("awaiting_finish")

        yield* prompt.command({
          sessionID: session.id,
          command: Command.Default.COMPOSE,
          arguments: `keep ${run.id} --confirmed`,
          agent: "compose",
        })
        expect((yield* Effect.promise(() => compose.get(run.id))).status).toBe("completed")
      }),
    ),
  { git: true },
  30_000,
...
it.instance(
  "keeps Compose Next in Grill until the model marks the specification ready",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const { llm } = yield* useServerConfig(providerCfg)
        ...
        const sessions = yield* Session.Service
        const test = yield* TestInstance
        const session = yield* sessions.create({ title: "Interactive Compose" })
        yield* llm.text("请先确认是否需要兼容旧配置。")

        yield* prompt.command({
          sessionID: session.id,
          command: Command.Default.COMPOSE_NEXT,
          arguments: "实现可恢复任务",
          agent: "compose",
          model: "test/test-model",
        })
        const compose = yield* Effect.promise(() => createComposeService({ directory: test.directory }))
        let run = (yield* Effect.promise(() => compose.list()))[0]
        expect(run.mode).toBe("interactive")
        expect(run.phase).toBe("grill")

        yield* llm.text("完整规格已就绪。\n<!-- workmesh-compose:spec-ready -->")
        yield* prompt.prompt({
          sessionID: session.id,
          agent: "compose",
          model: { providerID: ref.providerID, modelID: ref.modelID },
          parts: [{ type: "text", text: "需要兼容旧配置。" }],
        })
        run = yield* Effect.promise(() => compose.get(run.id))
...
it.instance(
  "loop command creates a job and executes it immediately",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const { llm } = yield* useServerConfig(providerCfg)
        ...
        yield* llm.text("首次循环执行完成")

        const result = yield* prompt.command({
          sessionID: session.id,
          command: Command.Default.LOOP,
          arguments: "60 检查当前构建状态",
          agent: "build",
          model: "test/test-model",
        })

        expect(result.info.role).toBe("assistant")
        expect(result.parts.some((part) => part.type === "text" && part.text === "首次循环执行完成")).toBe(true)
        expect(yield* llm.calls).toBe(1)
        expect(JSON.stringify(yield* llm.inputs)).toContain("检查当前构建状态")

        const jobs = yield* loops.list(session.id)
...
it.instance(
  "loop scheduler restores one overdue execution",
  () =>
    withWorkMesh(
      Effect.gen(function* () {
        const { llm } = yield* useServerConfig(providerCfg)
        ...
        const loops = yield* WorkMeshLoops.Service
        const sessions = yield* Session.Service
        const session = yield* sessions.create({ title: "Restored loop" })
        yield* loops.create({
          sessionID: session.id,
          prompt: "恢复后检查项目状态",
          intervalSeconds: 60,
        ...
        yield* llm.text("恢复执行完成")

        yield* prompt.init()
        yield* prompt.init()
        const restored = yield* pollWithTimeout(
          sessions
            .messages({ sessionID: session.id })
            .pipe(
              Effect.map((messages) =>
                messages.find((message) =>
                  message.parts.some((part) => part.type === "text" && part.text === "恢复执行完成"),
                ),
              ),
            ),
          "循环任务没有在恢复后执行",
