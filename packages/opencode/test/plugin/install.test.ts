...
describe("plugin.install.task", () => {
  test("writes WorkMesh plugins to the branded project config", async () => {
    ...
    try {
      ...
      expect((await read(path.join(tmp.path, ".workmesh", "config", "workmesh.jsonc"))).plugin).toEqual([
        "acme@1.2.3",
      ])
      expect((await read(path.join(tmp.path, ".workmesh", "config", "tui.jsonc"))).plugin).toEqual([
        "acme@1.2.3",
      ])
