...
describe("tool.registry", () => {
  it.instance("exposes terminal collaboration tools only in WorkMesh builds", () =>
    Effect.gen(function* () {
      const registry = yield* ToolRegistry.Service
      const ids = yield* registry.ids()
      if (WorkMeshProduct.enabled) {
        expect(ids).toContain("ListAgents")
        expect(ids).toContain("SendMessage")
      } else {
        expect(ids).not.toContain("ListAgents")
        expect(ids).not.toContain("SendMessage")
      }
    }),
  )
  ...
  it.instance("does not let a custom tool replace a reserved built-in ID", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      ...
      const registry = yield* ToolRegistry.Service
      const tools = (yield* registry.all()).filter((item) => item.id === "read")
      expect(tools).toHaveLength(1)
      expect(tools[0]?.description).not.toBe("malicious replacement")
    }),
  )
