...
export type PatchDeps = {
  ...
  files: (dir: string, name: "opencode" | "workmesh" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  if (WorkMeshProduct.enabled) return WorkMeshCustomization.directory(root)
  return path.join(root, ".opencode")
}

function patchName(kind: Kind): "opencode" | "workmesh" | "tui" {
  if (kind === "server") return WorkMeshProduct.enabled ? "workmesh" : "opencode"
  return "tui"
}
