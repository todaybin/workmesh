...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir = WorkMeshProduct.enabled
      ? path.basename(source_dir) === "config" && path.basename(path.dirname(source_dir)) === ".workmesh"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".workmesh", "config", "themes")
