    ...
    const init = Effect.fn("Command.state")(function* (ctx: InstanceContext) {
      ...
      if (WorkMeshProduct.enabled) {
        ...
        commands[Default.GOAL] = {
          name: Default.GOAL,
          description: "设置停止条件并持续执行，直到独立检查通过；使用 /goal clear 取消",
          source: "command",
          subtask: false,
          get template() {
            return "$ARGUMENTS"
        ...
        commands[Default.LOOPS] = {
          name: Default.LOOPS,
          description: "查看循环任务；使用 /loops <ID前缀> 取消",
          source: "command",
          subtask: false,
          get template() {
            return "$ARGUMENTS"
        ...
        commands[Default.MESSAGE] = {
          ...
          description: "向指定 Agent 发送短消息：/message <终端ID> <消息>",
          source: "command",
          subtask: false,
          get template() {
            return "$ARGUMENTS"
        ...
        commands[Default.COMPOSE] = {
          ...
          description: "启动、审批、恢复或完成可持久化 Compose 工作流",
          source: "command",
          subtask: false,
          get template() {
            return "$ARGUMENTS"
