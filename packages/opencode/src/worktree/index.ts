...
export interface Interface {
  ...
  readonly createScoped: (input: CreateScopedInput) => Effect.Effect<Info, Error>
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const setup = Effect.fnUntraced(function* (info: Info, baseRef?: string) {
      ...
      const created = yield* git(
        info.branch
          ...
          : ["worktree", "add", "--no-checkout", "--detach", info.directory, baseRef ?? "HEAD"],
        { cwd: ctx.worktree },
      )
      if (created.code !== 0) {
    ...
    const createFromInfo = Effect.fn("Worktree.createFromInfo")(function* (
      ...
      const bootstrap = boot(info, startCommand).pipe(
        Effect.catchCause((cause) => Effect.logError("worktree bootstrap failed", { cause })),
      )
      if (waitForBoot) yield* bootstrap
      else yield* bootstrap.pipe(Effect.forkIn(scope))
    })
    ...
    const createScoped = Effect.fn("Worktree.createScoped")(function* (input: CreateScopedInput) {
      const ctx = yield* InstanceState.context
      if (ctx.project.vcs !== "git") {
        return yield* new NotGitError({ message: "Worktrees are only supported for git projects" })
      }
      const root = pathSvc.resolve(input.root)
      const projectRoot = pathSvc.resolve(ctx.project.worktree)
      const relative = pathSvc.relative(projectRoot, root)
      if (relative.startsWith("..") || pathSvc.isAbsolute(relative)) {
        return yield* new CreateFailedError({ message: "Scoped worktree root must stay inside the project" })
      }
      yield* fs.makeDirectory(root, { recursive: true }).pipe(Effect.orDie)
      const prefix = input.branchPrefix.split("/").map(slugify).filter(Boolean).join("/")
      if (!prefix) return yield* new CreateFailedError({ message: "Scoped worktree branch prefix is required" })
      const base = slugify(input.name)
      if (!base) return yield* new CreateFailedError({ message: "Scoped worktree name is required" })
      const info = yield* candidate({ root, name: `${prefix.replaceAll("/", "-")}-${base}` })
      const scoped = { ...info, branch: `${prefix}/${base}` }
      const branchExists = yield* git(["show-ref", "--verify", "--quiet", `refs/heads/${scoped.branch}`], {
        cwd: ctx.worktree,
      })
      if (branchExists.code === 0) {
        return yield* new CreateFailedError({ message: `Worktree branch already exists: ${scoped.branch}` })
      }
      yield* createFromInfo(scoped, input.startCommand, input.baseRef, true)
      return scoped
    })
    ...
    const remove = Effect.fn("Worktree.remove")(function* (input: RemoveInput) {
      ...
      const removed = yield* git(["worktree", "remove", ...(input.force === false ? [] : ["--force"]), entry.path], {
        cwd: ctx.worktree,
      })
      if (removed.code !== 0) {
      ...
      if (branch && input.deleteBranch !== false) {
    ...
    return Service.of({ makeWorktreeInfo, createFromInfo, create, createScoped, list, remove, reset })
  }),
)
