...
const RESERVED_TOOL_IDS = new Set([
  "invalid", "question", "shell", "read", "glob", "grep", "edit", "write", "task", "webfetch", "websearch",
  "todowrite", "skill", "apply_patch", "lsp", "plan", "execute", "ListAgents", "SendMessage",
])
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        for (const match of matches) {
          ...
          for (const [id, def] of Object.entries(mod)) {
            ...
            if (!RESERVED_TOOL_IDS.has(toolID)) custom.push(fromPlugin(toolID, def))
          }
        }
        ...
        for (const p of plugins) {
          for (const [id, def] of Object.entries(p.tool ?? {})) {
            if (!RESERVED_TOOL_IDS.has(id)) custom.push(fromPlugin(id, def))
          }
        }
        ...
        return {
          ...
          builtin: [
            ...
            ...(WorkMeshProduct.enabled ? [tool.listAgents, tool.sendMessage] : []),
    ...
    const all: Interface["all"] = Effect.fn("ToolRegistry.all")(function* () {
      ...
      const ids = new Set(s.builtin.map((tool) => tool.id))
      const custom = s.custom.filter((tool) => {
        if (ids.has(tool.id)) return false
        ids.add(tool.id)
        return true
      })
      return [...s.builtin, ...custom] as Tool.Def[]
    })
    ...
    const describeCodeMode = Effect.fn("ToolRegistry.describeCodeMode")(function* (input: {
      ...
      if (!codeMode) return
      const ruleset = ComposePermission.merge(input.agent, input.permission ?? [])
