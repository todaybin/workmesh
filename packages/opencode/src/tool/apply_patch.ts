...
import { acquireWriteIntent, type WriteIntentLease } from "@/workmesh/write-intent"
...
export const ApplyPatchTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ApplyPatchTool.execute")(function* (
      ...
      ctx: Tool.Context,
      onLease?: (lease: WriteIntentLease) => void,
    ) {
      if (!params.patchText) {
      ...
      const writeIntent = yield* acquireWriteIntent(
        instance,
        ctx,
        fileChanges.flatMap((change) => [change.filePath, ...(change.movePath ? [change.movePath] : [])]),
      )
      onLease?.(writeIntent)
      yield* ctx.ask({
        permission: "edit",
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          let lease: WriteIntentLease | undefined
          return yield* run(params, ctx, (value) => {
            lease = value
          }).pipe(Effect.ensuring(Effect.promise(() => lease?.release() ?? Promise.resolve())), Effect.orDie)
        }),
    }
  }),
)
