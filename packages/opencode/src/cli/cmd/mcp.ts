...
async function resolveConfigPath(baseDir: string, global = false) {
  if (WorkMeshProduct.enabled) {
    const dir = global ? baseDir : WorkMeshCustomization.directory(baseDir)
    const candidates = WorkMeshCustomization.configWriteNames.map((name) => path.join(dir, name))
  ...
  // Official OpenCode keeps its upstream project and global configuration layout.
  ...
  // Default to opencode.jsonc if none exist.
  return candidates[0]
}
