  ...
  handler: Effect.fn("Cli.agent.create")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (cliPath) {
        ...
        const projectConfig = WorkMeshProduct.enabled
          ? WorkMeshCustomization.directory(ctx.worktree)
          : path.join(ctx.worktree, ".opencode")
        targetPath = path.join(scope === "global" ? Global.Path.config : projectConfig, "agents")
      }
