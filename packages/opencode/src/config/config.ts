...
function explicitConfigPath(value: string | undefined) {
  if (!value) return undefined
  if (WorkMeshProduct.enabled && WorkMeshCustomization.isLegacyPath(value)) return undefined
  return value
}
...
function globalConfigFile() {
  const names = WorkMeshProduct.enabled
    ? [...WorkMeshCustomization.configWriteNames, "config.json"]
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!customConfig && !customDirectory && !Flag.OPENCODE_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if ((!WorkMeshProduct.enabled && dir.endsWith(".opencode")) || dir === customDirectory) {
            const names = WorkMeshProduct.enabled
