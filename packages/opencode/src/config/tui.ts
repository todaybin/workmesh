...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const explicitTuiConfig = Flag.OPENCODE_TUI_CONFIG
  if (explicitTuiConfig && (!WorkMeshProduct.enabled || !WorkMeshCustomization.isLegacyPath(explicitTuiConfig))) {
  ...
  const explicitConfigDir = Flag.OPENCODE_CONFIG_DIR
  const customDirectory =
    explicitConfigDir && (!WorkMeshProduct.enabled || !WorkMeshCustomization.isLegacyPath(explicitConfigDir))
      ? explicitConfigDir
      : undefined
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === customDirectory)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== customDirectory) continue
