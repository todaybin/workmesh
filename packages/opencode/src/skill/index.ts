...
const CUSTOMIZE_OPENCODE_SKILL_BODY = WorkMeshProduct.enabled
  ? SkillPlugin.CustomizeWorkMeshContent
  : SkillPlugin.CustomizeOpencodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (WorkMeshProduct.enabled) {
    for (const root of WorkMeshBuiltinSkills.directories({
      executable: process.execPath,
      configured: process.env.WORKMESH_BUILTIN_SKILLS_DIR,
      directory,
    })) {
      if (!(yield* fsys.isDir(root))) continue
      yield* scan(state, root, SKILL_PATTERN, { scope: "WorkMesh built-in" })
    }
  }
