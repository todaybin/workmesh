...
export type GoalStatus = "active" | "paused" | "stalled" | "complete"
...
async function loadGoals() {
  ...
  return new Map(
    ...
    ).map(([key, value]) => [key, { ...value, status: value.status ?? ("active" as const) }]),
  )
}
...
export interface Interface {
  ...
  readonly setStatus: (sessionID: SessionID, status: GoalStatus) => Effect.Effect<void>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const set = Effect.fn("SessionGoal.set")(function* (sessionID: SessionID, condition: string) {
      const data = yield* InstanceState.get(state)
      data.goals.set(sessionID, { condition, react: 0, status: "active" })
      yield* persist()
    })
    ...
    const patchStatus = Effect.fn("SessionGoal.patchStatus")(function* (
      sessionID: SessionID,
      status: GoalStatus,
    ) {
      const data = yield* InstanceState.get(state)
    ...
    const pause = Effect.fn("SessionGoal.pause")(function* (sessionID: SessionID) {
      yield* patchStatus(sessionID, "paused")
    })

    const resume = Effect.fn("SessionGoal.resume")(function* (sessionID: SessionID) {
      yield* patchStatus(sessionID, "active")
    })

    const setStatus = Effect.fn("SessionGoal.setStatus")(function* (sessionID: SessionID, status: GoalStatus) {
      yield* patchStatus(sessionID, status)
    })
    ...
    return Service.of({ set, get, clear, bumpReact, pause, resume, setStatus, evaluate })
  }),
)
