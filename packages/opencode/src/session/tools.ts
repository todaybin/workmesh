...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
    ...
    ask: (req) =>
      permission
        .ask({
          ...
          ruleset: ComposePermission.merge(input.agent, input.session.permission ?? []),
        })
        .pipe(Effect.orDie),
  })
  ...
  for (const [key, entry] of Object.entries(yield* mcp.tools())) {
    if (["invalid", "question", "shell", "read", "glob", "grep", "edit", "write", "task", "webfetch", "websearch", "todowrite", "skill", "apply_patch", "lsp", "plan", "execute", "ListAgents", "SendMessage"].includes(key)) continue
    ...
    registerExternalTool(tools, key, item)
  }

  return tools
})

export function registerExternalTool<T>(tools: Record<string, T>, id: string, item: T) {
  // Built-in and registered tools own their IDs. External providers must not
  // replace an allowed implementation by choosing the same sanitized name.
  if (Object.hasOwn(tools, id)) return false
  tools[id] = item
  return true
}
