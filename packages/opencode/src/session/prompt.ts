        ...
        if (firstUser?.info.role === "user") {
          ...
          if (composeLocked !== enteringCompose) {
            ...
            const message =
              language === "zh-CN"
                ? composeLocked
                  ? "此会话已锁定�?Compose 模式，后续消息必须继续使�?Compose�?
      ...
      yield* Effect.gen(function* () {
        while (true) {
          yield* loops.claimDue().pipe(
            ...
            Effect.catchCause((cause) => Effect.logWarning("WorkMesh 循环调度检查失�?, { cause })),
          )
                ...
                if (attempt < MAX_GOAL_REACT) {
                  ...
                  yield* sessions.updatePart({
                    ...
                    text: [
                      ...
                      `继续完成任务。这是最�?${MAX_GOAL_REACT} 次中的第 ${attempt} 次续跑。`,
                      "</system-reminder>",
                    ].join("\n"),
                  } satisfies SessionV1.TextPart)
                  return "continue" as const
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
        ...
        if (input.command === Command.Default.MESSAGE) {
          ...
          if (!match) {
            return yield* respond(
              text(
                "请在 TUI 中直接输�?/message，通过弹窗选择终端并填写消息。\n脚本用法�?message <终端ID> <消息>",
          ...
          return yield* respond(
            text(
              `消息已发送到 ${sent.recipientTerminalId}，消�?ID�?{sent.id}。`,
      ...
      if (
        ...
        const resolveRun = Effect.fnUntraced(function* (id?: string) {
          ...
          if (!run)
            throw new Error(composeText("当前会话没有可用�?Compose 运行", "This session has no available Compose run"))
          if (run.sessionID && run.sessionID !== input.sessionID)
            throw new Error(
              composeText("Compose 运行不属于当前会�?, "The Compose run does not belong to this session"),
            )
          return run
        })
          ...
          Effect.gen(function* () {
            ...
            const reportPrompt = Effect.fnUntraced(function* () {
              const result = yield* runPrompt(
                "compose-review",
                composeText(
                  `生成最终交付报告：列出实际改动、任务结果、验证结果、Review 结论、剩余风险和最多五�?Journey Log。已批准规格�?{approvedSpecPath}\n验证摘要�?{current.verificationSummary}\nReview 摘要�?{current.reviewSummary}\n不得执行任何 Git 收尾操作。`,
            ...
            const awaitFinish = Effect.fnUntraced(function* () {
              ...
              if (!current.git.reviewedTreeHash || digest !== current.git.reviewedTreeHash) {
                throw new Error(
                  composeText(
                    "Compose 工作树在 Review 通过后发生变化，必须重新验证�?Review�?,
            ...
            const applyPendingFix = Effect.fnUntraced(function* () {
              ...
              yield* runPrompt(
                ...
                kind === "review"
                  ? composeText(
                      `修复独立 Review 发现�?Critical 问题，不得扩大范围：\n${fixes.join("\n")}`,
            ...
            if (!current.tasks.length) {
              const planned = structured(
                yield* runPrompt(
                  "compose-review",
                  composeText(
                    `读取已批准规�?${approvedSpecPath}，提取可执行任务 DAG。ID 必须稳定且唯一，dependsOn 只能引用本清�?ID，验收条件必须可观察，files 列出预计写入范围。不得修改文件。`,
            ...
            if (current.phase === "implement") {
              while (current.tasks.some((task) => task.status !== "completed")) {
                ...
                if (!wave.length)
                  throw new Error(
                    composeText("Compose 任务 DAG 没有可执行节�?, "The Compose task DAG has no executable node"),
                  )
                        ...
                        parts: [
                          {
                            ...
                            text: [
                              ...
                              composeText(
                                "只允许写入任�?files 声明的范围。不�?commit、push、merge 或删�?Worktree/分支�?,
            ...
            const verify = Effect.fnUntraced(function* () {
              while (true) {
                ...
                yield* composeAsync(() => service.assertExecutionLease(current.id, lease))
                const summary = (result.stderr.toString("utf8").trim() || result.text.trim() || "验证无输�?).slice(
                  -4000,
                )
                if (result.code === 0) {
                  current = yield* update(
            ...
            while (true) {
              ...
              const result = yield* runPrompt(
                "compose-review",
                composeText(
                  `你是未参与实现的全新独立审查 Agent。读取已批准规格 ${approvedSpecPath} 和精确证�?${evidence}，检查规格符合性、正确性和代码库一致性（Critical 修复轮次 ${round}/2）。Status 中的未跟踪文件需直接读取。只报告可验证问题并返回结构化结果。`,
              ...
              current = yield* update(
                service.update(current.id, "review-evaluated", (run) => ({
                  ...run,
                  reviewSummary: String(data.summary ?? "Review 未提供摘�?),
                }), lease),
              )
              ...
              if (blocking.length === 0)
                throw new Error(
                  composeText(
                    `Compose 独立 Review 未确认可交付�?{String(data.summary ?? "未提供原�?)}`,
              ...
              if (round >= 2)
                throw new Error(
                  composeText(
                    `阻断 Review 问题�?2 轮修复后仍存在：${blocking.join("�?)}`,
            ...
            parts: [
              {
                ...
                text: [
                  composeText(
                    interactiveCompose
                      ? "启动可持久化的交互式 Compose Next 工作流。先加载并遵�?compose-next Skill�?
                      : "启动可持久化 Compose 工作流。先加载并遵�?compose-next Skill�?,
                    interactiveCompose
                  ...
                  composeText(
                    interactiveCompose
                      ? "当前只完�?Orient、Grill �?Spec：检查项目上下文，逐项澄清关键决策，提出规格、验收条件、任务依赖和验证方案�?
                      : "当前只完�?Brainstorm �?Design：检查项目上下文，澄清必要假设，提出规格、验收条件、任务依赖和验证方案�?,
                    interactiveCompose
                  ...
                  composeText(
                    "不要修改任何产品代码，不�?commit、push、merge，不要创建或删除 Worktree/分支。请在回复末尾明确请求用户审批规格�?,
        ...
        if (operation === "cancel") {
          ...
          return yield* notice(
            composeText(
              `已取�?Compose 运行 ${cancelled.id}，可使用 /compose resume ${cancelled.id} 恢复。`,
            ...
            parts: [
              {
                type: "text",
                text: composeText(
                  `请按以下审批意见修订 Compose 规格并重新请求审批。仍不得修改产品代码或执�?Git 写操作。\n\n${instruction}`,
            ...
            (lease) =>
              Effect.gen(function* () {
                ...
                return yield* continueToFinish(
                  approved,
                  composeText("用户已批�?Compose 规格�?, "The user approved the Compose specification."),
                  lease,
                )
              }),
            ...
            (leased) =>
              Effect.gen(function* () {
                ...
                return yield* continueToFinish(
                  resumed,
                  composeText(
                    `恢复 Compose 运行 ${resumed.id}，从持久化阶�?${resumed.phase} 继续。`,
      ...
      if (input.command === Command.Default.LOOPS) {
        ...
        if (value) {
          ...
          return yield* prompt({
            ...
            messageID: input.messageID,
            agent: agentName,
            parts: [{ type: "text", text: `已取消循环任�?${job.id}。`, synthetic: true }],
            noReply: true,
          })
        }
        ...
        const text =
          jobs.length === 0
            ...
            : [
                ...
                ...jobs.map(
                  (job) =>
                    `- ${job.id} | �?${job.intervalSeconds} �?| 下次 ${formatLoopTime(job.nextRunAt)} | ${summarizeLoopPrompt(job.prompt)}`,
                ),
              ].join("\n")
        return yield* prompt({
          sessionID: input.sessionID,
      ...
      if (input.command === Command.Default.GOAL) {
        ...
        if (normalized === "clear" || normalized === "reset" || normalized === "取消") {
          ...
          return yield* prompt({
            ...
            messageID: input.messageID,
            agent: agentName,
            parts: [{ type: "text", text: "已取消当�?Goal�?, synthetic: true }],
            noReply: true,
          })
        }
        if (normalized === "pause") {
          ...
          return yield* prompt({
            ...
            messageID: input.messageID,
            agent: agentName,
            parts: [{ type: "text", text: "已暂停当�?Goal。使�?/goal resume 恢复�?, synthetic: true }],
            noReply: true,
          })
        }
        ...
        if (!condition) {
          const current = yield* goal.get(input.sessionID)
          const label = current
            ? `当前 Goal�?{current.condition}�?{current.status === "stalled" ? "已挂�? : current.status === "paused" ? "已暂�? : current.status === "complete" ? "已完�? : "进行�?}）`
            : "当前没有 Goal。使�?/goal <目标> 设置停止条件�?
          return yield* prompt({
            sessionID: input.sessionID,
            messageID: input.messageID,
            agent: agentName,
...
function parseLoopArguments(value: string) {
  ...
  if (
    ...
    throw new Error(
      `循环周期必须�?${WorkMeshLoops.MIN_INTERVAL_SECONDS} �?${WorkMeshLoops.MAX_INTERVAL_SECONDS} 秒之间`,
    )
  }
