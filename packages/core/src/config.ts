...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = WorkMeshRuntimeLayout.enabled
      ? WorkMeshCustomization.configLoadNames
      : (["opencode.json", "opencode.jsonc"] as const)
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: WorkMeshRuntimeLayout.enabled ? names.toReversed() : [".opencode", ...names.toReversed()],
    ...
    // global config, project files, then product-specific configuration directories.
