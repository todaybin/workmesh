...
describe("MoveSession", () => {
  it.live("moves a session from main to a worktree and back", () =>
    Effect.gen(function* () {
      ...
      const main = abs(yield* Effect.promise(() => fs.realpath(root.path)))
      const destination = abs(`${root.path}-round-trip-worktree`)
      ...
      const worktree = abs(yield* Effect.promise(() => fs.realpath(destination)))

      const projectID = (yield* Project.Service.use((service) => service.resolve(main))).id
      const sessionID = SessionV2.ID.make("ses_move_round_trip")
      const { db } = yield* Database.Service
      yield* db
        .insert(ProjectTable)
        .values({ id: projectID, worktree: main, sandboxes: [], time_created: 1, time_updated: 1 })
        .run()
        .pipe(Effect.orDie)
      yield* db
        .insert(SessionTable)
      ...
      yield* MoveSession.Service.use((service) =>
        Effect.gen(function* () {
          yield* service.moveSession({ sessionID, destination: { directory: worktree } })
          expect((yield* SessionStore.Service.use((store) => store.get(sessionID)))?.location.directory).toBe(worktree)
          yield* service.moveSession({ sessionID, destination: { directory: main } })
        }),
      )
